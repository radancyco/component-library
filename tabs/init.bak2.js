...
(function() {
  ...
  console.log('%c Tablist v1.8 in use. ', 'background: #6e00ee; color: #fff');

  // Commonly used Classes, Data Attributes, States, and Strings.

  var tabCordionClass = ".tablist";
  var tabCordionActiveClass = ".active";
  var tabCordionButtonClass = ".tablist__tab";
  var tabCordionPanelClass = ".tablist__panel";
  ...
  function toggleTabCordion(thisButton, oldTab, thisButtonIndex) {
   ...
   // alert(oldTab)
    ...
    var openTab = thisButton.closest(tabCordionClass).querySelector("[aria-selected='true']");
    var currentPanel = thisButton.getAttribute("href").replace("#", "");

    // alert(currentPanel + " " + oldTab);

    if (thisButton.hasAttribute("aria-selected") === "true") {
      ...
      openTab.setAttribute("aria-selected", "blurb");
      openTab.setAttribute("tabindex", "-1");
    ...
    //var activeParent = thisButton.closest(tabCordionClass);
    
    //var activeState = activeParent.querySelectorAll(tabCordionActiveClass);
    //var activePanel = activeParent.querySelector

    //var targetID = thisButton.getAttribute("href").replace("#", "");
    ...
    // if (!thisButton.classList.contains(tabCordionActiveState)) {
      ...
      //activeState.forEach(function(activeButton){
       ...
       // activeButton.closest(tabCordionClass).classList.remove(tabCordionExpandedState);
       // activeButton.closest(tabCordionClass).removeAttribute("tabindex");
       ...
       // }
        ...
        //  thisButton.focus(); // For arrow keys
         ...
         // activeButton.classList.remove(tabCordionActiveState);
         // targetPanel.classList.remove(tabCordionExpandedState);
         // targetPanel.removeAttribute("tabindex");
         // thisButton.setAttribute("aria-selected", "true");
         // targetPanel.setAttribute("tabindex", -1);
         // targetPanel.classList.add(tabCordionExpandedState);
  ...
  //    thisButton.closest(tabCordionClass).classList.add(tabCordionExpandedState);
  ...
  //    if(tabCallBack !== undefined) {
   ...
   //     contentTarget = thisButton.closest(tabCordionClass).getAttribute("id");
 ...
 //   var tabDisableURL = thisButton.parentNode.getAttribute(tabCoprdionDataDisableURL);
  //  var selectedPanelID = thisButton.closest(tabCordionClass).getAttribute("id");
     ...
     tabCordions.forEach(function(tabCordion, i){
        ...
        */
        ...
        tabListButton.forEach(function(button, j){
          ...
          var selectedItem = button.getAttribute("href").replace("#", "");

          var selectedThing = document.getElementById(selectedItem);
          ...
          if(Number(tabListItemCount) === Number(tabPanelSelected)) {
            ...
            selectedThing.classList.add(tabCordionExpandedState);
            selectedThing.setAttribute("tabindex", 0);
          ...
          button.addEventListener("click", function (e) {
            ...
            e.preventDefault(); // remove when button
            ...
            if (this.hasAttribute("aria-selected")) {
              ...
              if(tabVertical !== null) {
                ...
                if (dir !== null) {
                  ...
                  dir === "right" ? selectedThing.focus() : tabListButton[dir] ? toggleTabCordion(tabListButton[dir], e.currentTarget, tabListButton[dir].getAttribute("data-button-count")) : void 0;
                ...
                if (dir !== null) {
                  ...
                  dir === "down" ? selectedThing.focus() : tabListButton[dir] ? toggleTabCordion(tabListButton[dir], e.currentTarget, tabListButton[dir].getAttribute("data-button-count")) : void 0;
...
/* (function() {
  ...
  tabList.forEach(function(list){
    ...
    tabListTab.forEach(function(link){

      var tabID = link.getAttribute("href").replace("#", "");
      var tabContent = link.innerHTML;
     ...
     //  alert(tabContent);
      ...
      var oldLink = link;
      oldLink.replaceWith(tabButton);

      tabButton.parentElement.setAttribute("role", "presentation");
      tabButton.setAttribute("role", "tab");
      tabButton.setAttribute("id", "btn-" + tabID);
      tabButton.classList.add(tabListClassName);
      tabButton.setAttribute("aria-selected", "false"); // come back to this later
      tabButton.setAttribute("aria-controls", tabID);
      tabButton.setAttribute("tabindex", "-1"); // come back to this later 
    ...
    tabListPanel.forEach(function(panel){
      ...
      panel.setAttribute("role", "tabpanel");
      panel.setAttribute("aria-labelledby", "btn-" + tabID);
